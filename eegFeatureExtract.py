################################################
#    This file is from the following repository and references the following paper:
...
def pfvecbandpower(data, fs, fband):
    ...
    for i in range(data.shape[0]):
        freqs, powers = signal.periodogram(data[i, :, :], fs, axis=0)
        idx_min = np.argmax(freqs > fband[0]) - 1
        idx_max = np.argmax(freqs > fband[1]) - 1
        idx_delta = np.zeros(dtype=bool, shape=freqs.shape)
        idx_delta[idx_min:idx_max] = True
       
        bpower = integrate.simps(powers[idx_delta, :], freqs[idx_delta], axis=0)
        bpowers[i, :] = bpower
...
def tsalisEntropy(eegData, bin_min, bin_max, binWidth, orders = [1]):
    ...
    for chan in tqdm(range(H[0].shape[0])):

        for epoch in range(H[0].shape[1]):
            counts, bins = np.histogram(eegData[chan,:,epoch], bins=np.arange(bin_min+1, bin_max, binWidth))
            # print('counts',counts)
            # print ('bins',bins)
            pmf = counts / sum(counts)
            # print ('pmf',pmf)
            dist = dit.Distribution([str(bc).zfill(5) for bc in bins[:-1]],pmf)
...
def mfcc(eegData,fs,order=2):
    ...
    for chan in range(H.shape[0]):
        for epoch in range(H.shape[1]):
            H[chan, epoch, : ] = librosa.feature.mfcc(y=np.asfortranarray(eegData[chan,:,epoch]), sr=fs, n_fft=fs*2)[0:order].T
    return H
...
def arma(eegData,order=2):
    H = np.zeros((eegData.shape[0], eegData.shape[2],order))
    for chan in tqdm( range(H.shape[0])):
        for epoch in range(H.shape[1]):
            arma_mod = ARIMA(eegData[chan,:,epoch], order=(order,order,0))
            #(eegData[chan,:,epoch], order=(order,order))
...
def diffuseSlowing(eegData, Fs=125, fast=True):
